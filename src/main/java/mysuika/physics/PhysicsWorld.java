...
public class PhysicsWorld {
	...
	// 現在落下中のフルーツの実体（衝突監視用）
	private Body dropfruit;
	// 物理ワールド内に存在する全フルーツのリスト
	private List<Body> fruits = new ArrayList<>();
	// 削除予定のフルーツ（合体や消去時に使用）
	private Set<Body> toBeRemoved = new HashSet<>();
	// 合体後に生成予定のフルーツ情報
	private List<FruitBlueprint> toBeAdded = new ArrayList<>();
	...
	private void mergeFruits(int typeA,Body a,Body b) {
		...
		// 1段階上のフルーツ生成予約
		toBeAdded.add(new FruitBlueprint(pos.x, pos.y, typeA + 1));
		// 元のフルーツは削除予約
		toBeRemoved.add(a);
		toBeRemoved.add(b);
	}
